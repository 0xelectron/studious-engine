...
function chart()
{
    ...
    for (var i = 0; i < shuttle.seats.length; i++)
    {
        ...
        {
            html += "<li id = " + shuttle.seats[i].house + ">" + shuttle.seats[i].name + "  to " + shuttle.seats[i].house + "</li>";
        }
...
function dropoff()
{
    ...
    for( var i in HOUSES)
    {
            ...
            var d = shuttle.distance(HOUSES[i].lat, HOUSES[i].lng);
                ...
                for ( var k = 0; k<shuttle.seats.length ; k++)
                {
                    if(shuttle.seats[k] != null)
                    {
                        ...
                        {  
                             ...
                             SCORE++;
                             $("#announcements").html("Dropped off!!  " + "   Your Score: " + SCORE);
    ...
    if(in_range == false)
    {
        $("#announcements").html("No one wants to get Droped off here!!");   
    }
...
function keystroke(event, state)
{
    ...
    else if (event.keyCode == 69 || event.keyCode == 101)
    {
        ...
        if (VELOCITY == 300)
        {
             alert("you are at max velocity!!!   Better drive carefully!!!"); 
        }
        
        else
            {
                VELOCITY = VELOCITY + 2;    
                $("#velocity").html("VELOCITY: " + VELOCITY); 
        }
        return false;
    }
    ...
    else if (event.keyCode == 81 || event.keyCode == 113)
    {
        ...
        if (VELOCITY == 0)
        {
           alert("your velocity has be reset to default!! You probably didn't want to drive so slow");   
           VELOCITY = 50;  
        }
        else
            {
...
function pickup()
{
    ...
    for( var i in PASSENGERS)
    {
       ...
       { 
            var passenger_lat = PASSENGERS[i].placemark.getGeometry().getLatitude(); 

            var passenger_lng = PASSENGERS[i].placemark.getGeometry().getLongitude(); 
    
            var d = shuttle.distance(passenger_lat, passenger_lng);
            ...
            if ( d <= 15 )
            {
                ...
                if ( seats == false)
                {
                    $("#announcements").html("no more seats!!!");
                } 
            }
    ...
    if(in_range == false)
    {
        $("#announcements").html("No one wants to get picked up here!!");   
    }
